...
public class MainViewModel extends AndroidViewModel {
    ...
    public MainViewModel(@NonNull Application application, ImagesDataRepo imagesDataRepo) {
        super(application);
        ...
        mResultsState = Transformations.switchMap(mQuery, (query) -> {
            ...
            return Transformations.map(mImagesDataRepo.getPhotos(query), (val) -> {
                if (val.data == null) {
        ...
        mLoadMoreState = Transformations.switchMap(mLoadMoreWithQuery, (query) ->
                Transformations.map(mImagesDataRepo.getNextPage(query, mCurrentPage), (val) -> {
            if (val.data == null) {
                return new Pair<>(val, query);
            }
            injectUrl(val.data);
